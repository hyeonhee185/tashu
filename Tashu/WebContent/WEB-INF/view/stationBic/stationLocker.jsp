...
function goBack(){
	location.replace("<%=request.getContextPath()%>/payment/paymentRent.do");
}
function updateRental(a,b,c){
		var result = confirm("자전거를 빌리시겠습니까?");
		if(result){
			location.replace("<%=request.getContextPath()%>/rental/rentUpdate.do?station_id="+a+"&locker_id="+b+"&bicycle_no="+c);
		}else{
...
%>
		<td class="td2">	
		<input type="button" id="btn" name="<%=bicVO.getBicycle_no() %>" onclick="updateRental(<%=bicVO.getStation_id() %>,<%=bicVO.getLocker_id() %>, <%=bicVO.getBicycle_no() %>)" >
<%
	}
%>
		</td>
