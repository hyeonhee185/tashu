...
<%-- 	$.ajax({
		...
		success : function(result){
			var str ="";
			str +="<table border='1'>";
			str +="<tr><th>정류소 번호</th><th>정류소 이름</th><th>반납가능한 라커</th></tr>";
			$.each(result, function(i,v){
				str += '<tr><td>';
				str += '<a href = "<%=request.getContextPath()%>/stationBic/returnBic.do?station_id='+v.station_id+'">'
				str += +v.station_id+'</td>';
				str += '<td>' + v.station_name + '</td>';
				str += '<td>' + v.cnt+' / '+v.station_locker + '</td></tr>';
			})
			str += "</table>";
			$("#result").html(str);
		},
