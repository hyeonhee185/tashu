...
<%@page import="kr.or.ddit.rental.vo.RentalVO"%>
...
<link rel="stylesheet" href="/Tashu/css/rentalList.css">
</head>
<body>
<header>
    <div class="headUtil"> 
...
%>
						...
						<li id="depth_1" class="on"><a id="use" href="<%=request.getContextPath() %>/rental/rentList.do"> 대여이력 </a></li>
					</ul>
				</div>
			</div>
<%
	List<RentalVO> rentList = 
		(List<RentalVO>)request.getAttribute("rentList");
...
if(rentList.size()==0){
%>
	<img src="<%=request.getContextPath() %>/images/rent.png" style="width : 400px"><br>
	대여 이력이 없습니다.
<%	
}else{
%>
...
<table border="1">
	<tr>
		<th>대여 시간</th><th>대여 장소</th><th>반납 시간</th><th>반납 장소</th>
	</tr>
<% 
for(RentalVO rentVO : rentList){
%>
	<tr>
		<td><%=rentVO.getRental_time() %></td>
		<td><%=rentVO.getRental_name() %></td>
<%
if(rentVO.getReturn_time()==null){
%>
	<td>-</td>
	<td>-</td>
<%
...
%>
		<td><%=rentVO.getReturn_time() %></td>
		<td><%=rentVO.getReturn_name() %></td>
<%
}
%>
	</tr>
